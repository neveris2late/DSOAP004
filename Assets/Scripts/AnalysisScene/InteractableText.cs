...
public class InteractableText : MonoBehaviour, IPointerClickHandler, IPointerMoveHandler, IPointerExitHandler
{
    ...
    public void OnPointerClick(PointerEventData eventData)
    {
        if (currentLinkIndex != -1)
        {
            ...
            StartCoroutine(SpawnAndFlyCharacters(linkInfo, clueID, clueText));
        }
    }

    private IEnumerator SpawnAndFlyCharacters(TMP_LinkInfo linkInfo, string clueID, string clueText)
    {
        ...
        // 确保你的 ClueManager 脚本已经挂载并存在 Instance
        if (ClueManager.Instance != null)
        {
            ClueManager.Instance.AddClueToPool(clueID, clueText);
        }
    }
